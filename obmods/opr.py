...
import inspect, os, time, threading
import olib

from olib import Object, get, save, update
...
def dne(event):
    ...
    db = Db()
    for o in db.find("obmods.opr.Todo", selector):
        o._deleted = True
        save(o)
        event.reply("ok")
        break
...
def ver(event):
    k = get_kernel()
    event.reply("OLIB %s" % olib.__version__)
    event.reply("OB %s" % __version__)
    for mod in k.walk("obmods"):
        try:
